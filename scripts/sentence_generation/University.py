...
table_index = np.array(
    category_map[category_map.category.isin(['University'])].table_id)
...
def get_Website(T, N, fake=False, sel=0):
            ...
            if (k in dictionary.keys()):
                ...
                if (len(dictionary[k].split(".")) > 3):
                    for i in range(2, len(dictionary[k].split("."))):
                        dom[dictionary["Tablename"]].append(
                            dictionary[k].split(".")[i])
                else:
                    dom[dictionary["Tablename"]].append(
                        dictionary[k].split(".")[-1])
            else:
                d[dictionary['Tablename']].append(None)
    ...
    if (fake):
        for it in range(37):  # for getting all the fakes in one go
            sel = random.sample(
                FakeDICT_helper["University"][k.replace(" ", "_")], 1)[0]
    ...
    for n in range(37):
        if (int(table_index[n][1:]) <= 2800):
            ...
            if (k in dictionary.keys()):
                ...
                if (type(dictionary[k]) == list):
                    for i in range(len(dictionary[k])):
                        u.add(dictionary[k][i].replace(",", "").replace(
                            "\xa0", "").replace("and", ",").lower())
                        d[dictionary['Tablename']].append(dictionary[k][i].replace(
                            ",", "").replace("\xa0", "").replace("and", ",").lower())
                else:

                    r = dictionary[k].replace(",", "").replace(
                        "\xa0", "").replace("and", ",").lower()
                    for i in r.split(","):
                        u.add(i.strip())
                ...
                if (type(dictionary[k]) == list):
                    ...
                    for i in range(len(dictionary[k].split(","))):
                        u.add(dictionary[k].split(",")[
                              i].replace("\xa0", " ").strip())
                        d[dictionary['Tablename']].append(
                            dictionary[k].split(",")[i].replace("\xa0", " ").strip())
...
def get_Data(fake=False):
    ...
    Extracted_data = {}
    Keys = ["Website", "Type", "Established", "Undergraduates", "Postgraduates", "Motto", "Location", "Nickname", "Campus", "Colors", "Students",
            "Academic_staff", "Administrative_staff", "President", "Endowment", "Mascot", "Provost", "Sporting_affiliations", "Academic_affiliations", "Former_names"]
    for k in Keys:
        Extracted_data[k] = []
    ...
    if (prem):
        if (di[tb[it]][0] != None and dom[tb[it]][0] != None):
            ...
            ps1 = [All+" is this"+random.sample(syn, 1)[0]+"'s website",
                   "This"+random.sample(syn, 1)[0] +
                   " has a website named "+All,
                   All+" is "+Nm+"'s website"]
        else:
            ps1 = [None]
        return ps1
        ...
        if (di[tb[it]][0] != None and dom[tb[it]][0] != None):
            ...
            if (tval):
                ...
                ts.append("The"+random.sample(syn, 1)
                          [0]+" "+Nm+" has the website "+All)
                ts.append("The website has domain name " +
                          random.sample(dom[tb[it]], 1)[0])
                ts.append(
                    "Information about the faculty or student could be found at "+All)
                ts.append(
                    "You can see the faculty or student profile of university "+Nm+" at "+All)
                ts.append(
                    "Admission info about the college could be found at "+All)
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                All = ','.join(di[tb[it]])
                ts.append(
                    Nm+" is a "+",".join(random.sample(di[tb[it]], random.randint(1, length)))+" university")
                ...
                ts.append("This university belongs to more than " +
                          str(random.randint(0, length-1))+" categories")
                ts.append("This university belongs to less than " +
                          str(random.randint(length+1, length+5))+" categories")
                ...
                nlen = len(NT)
                All = ','.join(NT)
                ts.append(Nm+" is a "+",".join(random.sample(NT,
                          random.randint(1, nlen)))+" university")
                if (re.findall("private", All)):
                    ts.append(Nm+" is a state-funded university")
                else:
                    ts.append(Nm+" is not a state-funded university")
                if (re.findall("public", All)):
                    ...
                    ts.append(Nm+" is not a self-funded university")
                ts.append(Nm+" belongs to " +
                          str(random.randint(length+1, length+5))+" categories")
                ts.append("This university belongs to less than " +
                          str(random.randint(1, length))+" categories")
                ts.append("This university belongs to more than " +
                          str(random.randint(length, length+5))+" categories")
...
def EstablishedSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = ["The university was established in "+All, All +
                   " is when this university was established", Nm+" was established in "+All]
        else:
            ps1 = [None]
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                ...
                ts.append(Nm+" was"+random.sample(syn, 1)
                          [0]+"before "+str(random.randint(year+10, 2020)))
                ts.append(Nm+" was"+random.sample(syn, 1)
                          [0]+"after "+str(random.randint(year-100, year-10)))
                ts.append(Nm+" is "+str(2020-year)+' year old university')
                ts.append(
                    "The founding stone of the university was laid in "+str(year))
                for i in jub.keys():
                    if (year+i <= 2020):
                        ts.append(Nm+" celebrated it's " +
                                  jub[i]+"jubilee on "+str(year+i))
                    else:
                        ts.append(Nm+" will celebrate it's " +
                                  jub[i]+"jubilee on "+str(year+i))
                ...
                ts.append(Nm+" was"+random.sample(syn, 1)
                          [0]+"after "+str(random.randint(year+10, 2020)))
                ts.append(Nm+" was"+random.sample(syn, 1)
                          [0]+"before "+str(random.randint(year-100, year-10)))
                ts.append(
                    Nm+" is "+str(2020-int(NT[0]))+' year old university')
                ts.append(
                    "The founding stone of the university was laid in "+NT[0])
                year = int(NT[0])
                for i in jub.keys():
                    if (year+i <= 2020):
                        ts.append(Nm+" celebrated it's " +
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                All = ','.join(di[tb[it]])
                ts.append(
                    str(num)+" bachelor students studied from this university")
                ts.append(
                    str(num)+" students are enrolled in the bachelor's program")
                ts.append("There are "+str(num) +
                          " undergraduate students on campus every year")
                ts.append("There are more than "+str(random.randint(num-80,
                          num-50))+" undergraduate students on campus every year")
                ts.append("There are less than "+str(random.randint(num+50,
                          num+100))+" undergraduate students on campus every year")
                ts.append(Nm+" is expected to graduate " +
                          str(num)+" bachelor students")
                ts.append(str(num)+" undergraduates study at "+Nm)

            else:
                NT = random.sample(list(set(univ)-set(di[tb[it]])), 1)
                ts.append(
                    NT[0]+" bachelor students studied from this university")
                ts.append(
                    NT[0]+" students are enrolled in the bachelor's program")
                ts.append("There are " +
                          NT[0]+" undergraduate students on campus every year")
                ts.append("There are less than "+str(random.randint(num-80,
                          num-50))+" undergraduate students on campus every year")
                ts.append("There are more than "+str(random.randint(num+50,
                          num+100))+" undergraduate students on campus every year")
                ts.append(Nm+" is expected to graduate " +
                          NT[0]+" bachelor students")
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                ...
                ts.append(
                    str(num)+" students are enrolled in the postgraduate program")
                ts.append("There are "+str(num) +
                          " P.G. students on campus every year")
                ts.append("There are more than "+str(random.randint(num-80,
                          num-50))+" postgraduate students on campus every year")
                ts.append("There are less than "+str(random.randint(num+50,
                          num+100))+" postgraduate students on campus every year")
                ts.append(Nm+" is expected to graduate " +
                          str(num)+" P.G. students")
                ...
                ts.append(
                    NT[0]+" students are enrolled in the postgraduate program")
                ts.append("There are " +
                          NT[0]+" postgraduate students on campus every year")
                ts.append("There are less than "+str(random.randint(num-80,
                          num-50))+" postgraduate students on campus every year")
                ts.append("There are more than "+str(random.randint(num+50,
                          num+100))+" postgraduate students on campus every year")
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                ...
                ts.append(random.sample(di[tb[it]], 1)[
                          0]+" are the words the university "+random.sample(["lives by ", "believes in "], 1)[0])
                ts.append(random.sample(di[tb[it]], 1)[
                          0]+" is the "+random.sample(["slogan ", "apopthegm ", "platitude "], 1)[0]+"of "+Nm)
                ts.append(
                    Nm+random.sample(["stand by ", "believe in "], 1)[0]+random.sample(di[tb[it]], 1)[0])
                ...
                ts.append(random.sample(NT, 1)[
                          0]+" is the "+random.sample(["slogan ", "apopthegm ", "platitude "], 1)[0]+"of "+Nm)
                ts.append(
                    Nm+random.sample(["stand by ", "believe in "], 1)[0]+random.sample(NT, 1)[0])
        ...
        if (diP[tb[it]][0] != None or diC[tb[it]][0] != None):
            if (tval):
                if (diP[tb[it]][0] != None):
                    ...
                    ts.append(Nm+" is"+random.sample(syn, 1)
                              [0]+random.sample(diP[tb[it]][0].split(",")[1:], 1)[0])
                if (diC[tb[it]][0] != None):
...
def NicknameSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = ["The nicknames of the students from this university are " +
                   All, All+" are the nicknames given to the students of "+Nm]
        else:
            ps1 = [None]
        return ps1
    else:
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                All = ','.join(di[tb[it]])
                ts.append('The students of '+Nm+" university are called " +
                          random.sample(di[tb[it]], 1)[0])
                ts.append(random.sample(di[tb[it]], 1)[
                          0]+" is graduated from "+Nm)
                ts.append("Students from "+Nm+" is known as " +
                          random.sample(di[tb[it]], 1)[0])
                ts.append("Another name for students of "+Nm +
                          " university is "+random.sample(di[tb[it]], 1)[0])
...
def CampusSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (diA[tb[it]][0] != None):
            ...
            if (len(diU[tb[it]]) != 0 and diU[tb[it]][0] != None):
                ps1 = ["The campus of university is "+All +
                       " ("+diU[tb[it]][0]+")", All+" ("+diU[tb[it]][0]+")"+" is the campus of this university"]
            else:
                ps1 = ["The campus of university is "+All,
        ...
        if (len(diA[tb[it]]) > 0 and diA[tb[it]][0] != None):
            if (tval):
                ts.append("The size of "+Nm+" is " +
                          random.sample(diA[tb[it]], 1)[0])
                ts.append(Nm+" is spread in "+random.sample(diA[tb[it]], 1)[0])
                ts.append(Nm+" has an area of " +
                          random.sample(diA[tb[it]], 1)[0])
                ts.append(Nm+random.sample(syn, 1)
                          [0]+random.sample(diA[tb[it]], 1)[0]+" area")
                if (len(diU[tb[it]]) != 0 and diU[tb[it]][0] != None):
                    ts.append(Nm+" is realized in "+diU[tb[it]][0])
                    ts.append(Nm+" is located in "+diU[tb[it]][0])
                l = random.sample(diA[tb[it]], 1)[
                    0].replace(",", "").split(" ")
                n = int(math.ceil(float(l[0])))
                ts.append(Nm+" is less than " +
                          str(random.randint(n+1, n+4))+" "+l[1])
                ts.append(Nm+" is more than " +
                          str(random.randint(0, n-1))+" "+l[1])

            else:
                NT = random.sample(
                    list(set(univA)-set(diA[tb[it]])), random.randint(1, 2))
                ...
                ts.append(Nm+random.sample(syn, 1)
                          [0]+random.sample(NT, 1)[0]+" area")
                if (len(diU[tb[it]]) != 0 and diU[tb[it]][0] != None and len(diU[tb[it]]) < 2):
                    ts.append(
                        Nm+" is realized in "+random.sample(list(set(univU)-set(diU[tb[it]])), 1)[0])
                    ts.append(
                        Nm+" is located in "+random.sample(list(set(univU)-set(diU[tb[it]])), 1)[0])
                l = random.sample(diA[tb[it]], 1)[
                    0].replace(",", "").split(" ")
                n = int(math.ceil(float(l[0])))
                ts.append(Nm+" is more than " +
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                All = ','.join(di[tb[it]])
                ts.append(random.sample(di[tb[it]], 1)[
                          0]+" is the school color of "+Nm)
                ts.append(random.sample(di[tb[it]], 1)[
                          0]+" is one of the school colors of this university")
                ts.append(random.sample(di[tb[it]], 1)[
                          0]+" can be seen at lot of place at "+Nm)
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                ...
                ts.append("There are more than "+str(random.randint(num -
                          80, num-50))+" students on campus every year")
                ts.append("There are less than "+str(random.randint(num +
                          50, num+100))+" students on campus every year")
...
def Academic_staffSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = [All+" academic staff are at " +
                   Nm, Nm+" has "+All+" academic staff"]
        else:
            ps1 = [None]
        return ps1
    else:
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                All = ','.join(di[tb[it]])
                ts.append("There are "+All +
                          " full time employees in the university")
                ts.append(Nm+" employees "+All+" people")
                ts.append(
                    All+" people  assist in the day to day activities at "+Nm)
                ts.append(All+" people work at "+Nm)

            else:
                NT = random.sample(list(set(univ)-set(di[tb[it]])), 1)
                ts.append("There are " +
                          NT[0]+" full time employees in the university")
                ts.append(Nm+" employees "+NT[0]+" people")
                ts.append(
                    NT[0]+" people  assist in the day to day activities at "+Nm)
...
def Administrative_staffSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = [All+" administrative staff are at " +
                   Nm, Nm+" has "+All+" administrative staff"]
        else:
            ps1 = [None]
        return ps1
    else:
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                ...
                ts.append(All+" makes the most "+random.sample(syn, 1)
                          [0]+" decision of the institution")
                ...
                ts.append(NT+" makes the most "+random.sample(syn, 1)
                          [0]+" decision of the institution")
...
def EndowmentSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = ["The university was endowed " +
                   All, All+" was endowed to "+Nm]
        else:
            ps1 = [None]
        return ps1
    else:
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                ...
                ts.append("The university "+Nm+random.sample(syn, 1)
                          [0]+All+" in donation")
                ts.append(Nm+random.sample(syn, 1)[0]+"more than "+"$" + str(
                    m-random.randint(1, min(int(m), 4)))+" "+mm)
                ts.append(Nm+random.sample(syn, 1)
                          [0]+"less than "+"$" + str(m+random.randint(1, 2))+" "+mm)
...
def MascotSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = ["The mascot of this university is " +
                   All, All+" is the mascot of "+Nm]
        else:
            ps1 = [None]
        return ps1
    else:
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                ...
                ts.append("You can see the symbol "+All +
                          " in every important event of "+Nm)
                ...
                ts.append("You can see the symbol "+NT +
                          " in every important event of "+Nm)
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                ...
                ts.append(
                    All+" is the second highest governing body of the university")
                ts.append(All+" makes the most "+random.sample(syn, 1)
                          [0]+" decision of the institution")
                ts.append(All+" is responsible for the university")
                ts.append(All+" is the administrator of the university")
                ...
                ts.append(
                    NT+" is the second highest governing body of the university")
                ts.append(NT+" makes the most "+random.sample(syn, 1)
                          [0]+" decision of the institution")
                ts.append(NT+" is responsible for the university")
                ts.append(NT+" is the administrator of the university")
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                ...
                ts.append("Sports body of the university " +
                          Nm+" is connected to "+All)
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                ...
                ts.append(Nm+" is"+random.sample(syn, 1)
                          [0]+All+" in academics")
                ts.append("Academic body of the university " +
                          Nm+" is connected to "+All)
...
def Former_namesSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = ["The former name of "+Nm+" was " +
                   All, Nm+" was formerly known as "+All]
        else:
            ps1 = [None]
        return ps1
    else:
    ...
    if (tval):
        if (ns != -1):
            if (S[tb[it]][0] != None):
                ...
                ts["Undergraduates,Postgraduates"].append(
                    "There are "+str(ns)+" students in "+Nm)
        if (nst != -1 and ns != -1):
            if (S[tb[it]][0] != None):
                ts["Students,Academic_staff,Administrative_staff"] = []
                ts["Students,Academic_staff,Administrative_staff"].append(
                    str(nst)+" employees manage "+str(ns)+" students")
            else:
                ts["Undergraduates,Postgraduates,Academic_staff,Administrative_staff"] = []
                ts["Undergraduates,Postgraduates,Academic_staff,Administrative_staff"].append(
                    str(nst)+" employees manage "+str(ns)+" students")
        ...
        if (ns != -1):
            if (S[tb[it]][0] != None):
                ts["Students"] = []
                ts["Students"].append(
                    "There are "+str(random.randint(ns+10, ns+1000))+" students in "+Nm)
            else:
                ts["Undergraduates,Postgraduates"] = []
                ts["Undergraduates,Postgraduates"].append(
                    "There are "+str(random.randint(ns+10, ns+1000))+" students in "+Nm)
        ...
        if (nst != -1 and ns != -1):
            if (S[tb[it]][0] != None):
                ts["Students,Academic_staff,Administrative_staff"] = []
                ts["Students,Academic_staff,Administrative_staff"].append(str(random.randint(
                    nst+10, nst+1000))+" employees manage "+str(random.randint(ns+10, ns+1000))+" students")
            else:
                ts["Undergraduates,Postgraduates,Academic_staff,Administrative_staff"] = []
                ts["Undergraduates,Postgraduates,Academic_staff,Administrative_staff"].append(str(random.randint(
                    nst+10, nst+1000))+" employees manage "+str(random.randint(ns+10, ns+1000))+" students")
...
def multi_row2(tb, dn, F, it, tval=True):
    ...
    if (tval):
        ...
        if (Pr[tb[it]][0] != None and Pv[tb[it]][0] != None):
            ...
            ts["President,Provost"].append(
                Al1+" and "+Al2+" are the main administrative figure heads of the university")
...
def multi_row3(tb, dn, F, it, tval=True):
    ...
    if (tval):
        if (len(ME) != 0 and len(MOt) != 0):
            ...
            ts["Motto"].append("English of the motto "+Al2 +
                               " of the university is "+Al1)
        if (S[tb[it]][0] != None and A[tb[it]][0] != None):
            ...
            ts["Sporting_affiliations,Academic_affiliations"].append(
                "Affiliations of the university are "+Al1+", "+Al2)
