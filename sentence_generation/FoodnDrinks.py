...
table_index = np.array(
    category_map[category_map.category.isin(['Food&Drink'])].table_id)
...
def get_Manufacturer(T, N, fake=False, sel=0):
    ...
    if (fake):
        for it in range(80):  # for getting all the fakes in one go
            sel = random.sample(
                FakeDICT_helper["FoodnDrinks"][k.replace(" ", "_")], 1)[0]
...
def get_Country_of_origin(T, N, fake=False, sel=0):
    ...
    if (fake):
        for it in range(80):  # for getting all the fakes in one go
            sel = random.sample(
                FakeDICT_helper["FoodnDrinks"][k1.replace(" ", "_")], 1)[0]
            if (sel == 2 and len(d[T[it]]) < 2):
                sel = 1
...
def get_Variants(T, N, fake=False, sel=0):
    ...
    if (fake):
        for it in range(80):  # for getting all the fakes in one go
            sel = random.sample(
                FakeDICT_helper["FoodnDrinks"]["Variants"], 1)[0]
            if (sel == 2 and len(d[T[it]]) < 2):
                sel = 1
            d = FakeDICT(T, N, u, d, it, sel)
        ...
        if (int(table_index[n][1:]) <= 2800):
            ...
            if (k in dictionary.keys()):
                ...
                if (type(dictionary[k]) == list):
                    for i in range(len(dictionary[k])):
                        year = re.findall(
                            "[0-9][0-9][0-9][0-9]", dictionary[k][i])[0]
                        u.add(year)
...
def get_Data(fake=False):
    ...
    Extracted_data = {}
    Keys = ["Manufacturer", "Country_of_origin", "Variants", "Introduced",
            "Related_products", "Alcohol_by_volume", "Website", "Color", "Main_ingredients", "Type"]
    for k in Keys:
        Extracted_data[k] = []
...
def ManufacturerSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            ps1 = [dn[tb[it]][0]+" is manufactured by "+di[tb[it]][0], "It is manufactured by " +
                   di[tb[it]][0], di[tb[it]][0]+" manufactured "+dn[tb[it]][0]]
        else:
            ps1 = [None]
        return ps1
    else:
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                ...
                ts.append(di[tb[it]][0]+" was made by " +
                          dn[tb[it]][0]+" company")
...
def Country_of_originSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = [All+" is the country of origin", All +
                   " is where it originated from", dn[tb[it]][0]+" originated from "+All]
        else:
            ps1 = [None]
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                ...
                ts = ['The food is from '+All,
                      ...
                      "It is made in less than " +
                      str(random.randint(
                          len(di[tb[it]])+1, len(di[tb[it]])+5))+" countries",
                      "It is made in more than " +
...
def VariantsSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = ["Variants of this food are "+All, All+" are the variants of " +
                   dn[tb[it]][0], "Several variants of this food are "+All]
        else:
            ps1 = [None]
        return ps1
    else:
        if (di[tb[it]][0] != None):
            if (tval):
                ...
                ts2 = "There are " + \
                    str(len(di[tb[it]]))+" flavours of this product"
                ts3 = "There are less than " + \
                    str(random.randint(
                        len(di[tb[it]])+1, len(di[tb[it]])+5))+" variants"
                ts4 = "There are more than " + \
                    str(random.randint(0, len(di[tb[it]])-1))+" variants"
                ts5 = random.sample(di[tb[it]], 1)[0] + \
                    " is one of the flavours of this"
                ts6 = ','.join(random.sample(di[tb[it]], random.randint(
                    1, len(di[tb[it]]))))+" are the variants of "+dn[tb[it]][0]
                ts7 = "There are " + \
                    ("single"if len(di[tb[it]]) ==
                     1 else "multiple") + " flavour"
                ...
                ts2 = "There are " + \
                    str(random.randint(
                        len(di[tb[it]])+1, len(di[tb[it]])+5))+" flavours of this product"
                ts3 = "There are more than " + \
                    str(random.randint(
                        len(di[tb[it]])+1, len(di[tb[it]])+5))+" variants"
                ts4 = "There are less than " + \
                    str(random.randint(0, len(di[tb[it]])-1))+" variants"
                ts5 = random.sample(NT, 1)[0]+" is one of the flavours of this"
                ts6 = ','.join(random.sample(NT, random.randint(
                    1, len(NT))))+" are the variants of "+dn[tb[it]][0]
                ts7 = "There are " + \
                    ("multiple"if len(di[tb[it]]) ==
                     1 else "single") + " flavour"
                ts8 = "It comes in " + \
                    str(random.randint(0, len(di[tb[it]])-1))+" flavours"
...
def IntroducedSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            ps1 = ["It was introduced in "+di[tb[it]][0], di[tb[it]][0] +
                   " is when it was introduced", dn[tb[it]][0]+" was introduced in "+di[tb[it]][0]]
        else:
            ps1 = [None]
        return ps1
    else:
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                ...
                ts2 = "It was " + \
                    random.sample(syn, 1)[0]+" after " + \
                    str(random.randint(year-50, year-10))
                ts3 = "It was " + \
                    random.sample(syn, 1)[0]+" before " + \
                    str(random.randint(year+10, year+60))
            else:
                NT = random.sample(list(set(univ)-set(di[tb[it]])), 1)[0]
...
def Related_productsSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = ["It is related to "+All, All+" are related to it",
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                ts.append(dn[tb[it]][0] + " is " + random.sample(syn, 1)[0] + " to " +
                          ','.join(random.sample(di[tb[it]], random.randint(1, length))))
                ts.append(dn[tb[it]][0] + " is " + random.sample(syn, 1)
                          [0] + " to "+str(length)+" products")
                ts.append(dn[tb[it]][0] + " is " + random.sample(syn, 1)
                          [0] + " to more than "+str(random.randint(0, length-1)))
                ts.append(dn[tb[it]][0] + " is " + random.sample(syn, 1)[0] +
                          " to less than "+str(random.randint(length+1, length+5)))
            else:
                NT = random.sample(
                    list(set(univ)-set(di[tb[it]])), random.randint(1, 4))
                ts.append(dn[tb[it]][0] + " is " + random.sample(syn, 1)[0] +
                          " to " + ','.join(random.sample(NT, random.randint(1, len(NT)))))
                ts.append(dn[tb[it]][0] + " is " + random.sample(syn, 1)[0] +
                          " to "+str(random.randint(length+1, length+3))+" products")
                ts.append(dn[tb[it]][0] + " is " + random.sample(syn, 1)[0] +
                          " to less than "+str(random.randint(0, length-1))+" products")
                ts.append(dn[tb[it]][0] + " is " + random.sample(syn, 1)[0] +
                          " to more than "+str(random.randint(length+1, length+5))+" products")
        else:
            ts.append(None)
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            ...
            ps1 = [dn[tb[it]][0]+" is "+All+"% by volume of alcohol",
                   "Alcohol is "+All+"% by volume in it", All+"% alcohol is present in it"]
        else:
            ps1 = [None]
        return ps1
    else:
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                ts.append(str(random.sample([mn, mx], 1)[
                          0])+"% is alcohol in "+dn[tb[it]][0])
                ts.append(str(100-mx)+"% is not alcohol")
                ts.append("It has more than " +
                          str(random.randint(mn-3, mn-1))+"% alcohol")
                ts.append("It has less than " +
                          str(random.randint(mx+5, mx+15))+"% alcohol")
                ts.append("Alcohol is the "+("major" if mx >=
                          50 else "not major")+"part of this beverage")
            else:
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                ...
                ts.append("This can be "+random.sample(syn, 1)
                          [0]+" from "+di[tb[it]][0])
                ts.append(
                    "This detail of the beverage can be found at "+di[tb[it]][0])
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            ...
            ps1 = [All+" are the colors of this food",
                   "Colors of this food are "+All, "The food comes in "+All]
        else:
            ps1 = [None]
        return ps1
    else:
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                ...
                ts.append("It does not come in " +
                          ",".join(random.sample(list(set(univ)-set(di[tb[it]])), 2)))
                ts.append("It come in "+str(len(di[tb[it]]))+" color(s)")
                ts.append("It comes in more than " +
                          str(random.randint(0, len(di[tb[it]])-1))+" color(s)")
                ts.append("It comes in less than " +
                          str(random.randint(len(di[tb[it]])+1, len(di[tb[it]])+4))+" color(s)")
                ts.append(
                    "It comes in "+("single color" if len(di[tb[it]]) == 1 else "multiple colors"))
            else:
                All = ','.join(di[tb[it]])
                ts.append("It does not come in "+All)
                ts.append(
                    "It comes in "+",".join(random.sample(list(set(univ)-set(di[tb[it]])), 2)))
                ts.append("It come in "+str(len(di[tb[it]]))+" color(s)")
                ts.append("It comes in less than " +
                          str(random.randint(0, len(di[tb[it]])-1))+" color(s)")
                ts.append("It comes in more than " +
...
def Main_ingredientsSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = ["The main ingredients of "+dn[tb[it]][0]+" are "+All, All +
                   " are the main ingredients of "+dn[tb[it]][0], "The main ingredients are "+All]
        else:
            ps1 = [None]
        return ps1
    else:
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                ...
                ts.append("More than " + str(random.randint(0, length-1)
                                             ) + " ingredients are required for "+dn[tb[it]][0])
                ts.append("Less than " + str(random.randint(length+1, length+4)
                                             ) + " ingredients are required for "+dn[tb[it]][0])
                ts.append(",".join(random.sample(
                    di[tb[it]], random.randint(1, length)))+" is used to make this")
                ts.append(",".join(random.sample(
                    di[tb[it]], 1))+" is not sufficient for making it")
                ts.append(",".join(random.sample(di[tb[it]], random.randint(
                    1, length)))+" are necessary to prepare this")
                ...
                ts.append("More than " + str(random.randint(length+1, length+4)
                                             ) + " ingredients are required for "+dn[tb[it]][0])
                ts.append(",".join(random.sample(
                    NT, random.randint(1, len(NT))))+" is used to make this")
                ts.append(",".join(random.sample(NT, 1)) +
                          " is not sufficient for making it")
                ts.append(",".join(random.sample(NT, random.randint(
                    1, len(NT))))+" are necessary to prepare this")
...
def TypeSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = ["The product is of type "+All, dn[tb[it]]
                   [0]+" is a "+All, "It is a type of "+All]
        else:
            ps1 = [None]
        return ps1
    else:
        ...
        if (di[tb[it]][0] != None):
            if (tval):
                ...
                ts.append("It is "+All)
                ts.append('It is not '+",".join(random.sample(list(set(univ) -
                          set(di[tb[it]])), random.randint(2, 5))))
...
def multi_row1(tb, dn, F, it, tval=True):
    ...
    if (tval):
        ...
        if (R[tb[it]][0] != None and V[tb[it]][0] != None):
            ...
            l1 = str(len(R[tb[it]]))
            l2 = str(len(V[tb[it]]))
            ts["Related_products,Variants"].append(
                "There are "+l1+" related products to this but "+l2+" variants")
            ts["Related_products,Variants"].append(
                "There are " + ("more" if int(l1) > int(l2) else "less")+" related products than variants")
        if (R[tb[it]][0] != None and M[tb[it]][0] != None):
            ts["Related_products,Main_ingredients"] = []
            l1 = str(len(R[tb[it]]))
            l2 = str(len(M[tb[it]]))
            ts["Related_products,Main_ingredients"].append(
                "There are "+l1+" related products to this but "+l2+" necessary ingrediets")
            ts["Related_products,Main_ingredients"].append("There are " + ("more" if int(
                l1) > int(l2) else "less")+" related products than necessary ingredients")
        if (R[tb[it]][0] != None and C[tb[it]][0] != None):
            ts["Related_products,Color"] = []
            l1 = str(len(R[tb[it]]))
            l2 = str(len(C[tb[it]]))
            ts["Related_products,Color"].append(
                "There are "+l1+" related products to this but it comes in "+l2+" colors")
            ts["Related_products,Color"].append("There are " + ("more" if int(
                l1) > int(l2) else "less")+" related products than color it comes in")
        # 2
        if (V[tb[it]][0] != None and M[tb[it]][0] != None):
            ts["Variants,Main_ingredients"] = []
            l1 = str(len(V[tb[it]]))
            l2 = str(len(M[tb[it]]))
            ts["Variants,Main_ingredients"].append(
                "There are "+l1+" variants of this but "+l2+" necessary ingredients")
            ts["Variants,Main_ingredients"].append(
                "There are " + ("more" if int(l1) > int(l2) else "less")+" variants than necessary ingredients")
        if (V[tb[it]][0] != None and C[tb[it]][0] != None):
            ts["Variants,Color"] = []
            l1 = str(len(V[tb[it]]))
            l2 = str(len(C[tb[it]]))
            ts["Variants,Color"].append(
                "There are "+l1+" variants of this but it comes in "+l2+" colors")
            ts["Variants,Color"].append(
                "There are " + ("more" if int(l1) > int(l2) else "less")+" variants than color it comes in")
        # 3
        if (M[tb[it]][0] != None and C[tb[it]][0] != None):
            ts["Main_ingredients,Color"] = []
            l1 = str(len(M[tb[it]]))
            l2 = str(len(C[tb[it]]))
            ts["Main_ingredients,Color"].append(
                "There are "+l1+" necessary ingredients to make this but it comes in "+l2+" colors")
            ts["Main_ingredients,Color"].append("There are " + ("more" if int(
                l1) > int(l2) else "less")+" main ingredients than color it comes in")
        ...
        if (R[tb[it]][0] != None and V[tb[it]][0] != None):
            ...
            l1 = str(len(R[tb[it]]))
            l2 = str(len(V[tb[it]]))
            ts["Related_products,Variants"].append("There are "+str(random.randint(int(l1)+1, int(
                l1)+4))+" related products to this but "+str(random.randint(int(l2)+1, int(l2)+5))+" variants")
            ts["Related_products,Variants"].append(
                "There are " + ("more" if int(l1) < int(l2) else "less")+" related products than variants")
        if (R[tb[it]][0] != None and M[tb[it]][0] != None):
            ts["Related_products,Main_ingredients"] = []
            l1 = str(len(R[tb[it]]))
            l2 = str(len(M[tb[it]]))
            ts["Related_products,Main_ingredients"].append("There are "+str(random.randint(int(l1)+1, int(
                l1)+4))+" related products to this but "+str(random.randint(int(l2)+1, int(l2)+2))+" necessary ingrediets")
            ts["Related_products,Main_ingredients"].append("There are " + ("more" if int(
                l1) < int(l2) else "less")+" related products than necessary ingredients")
        if (R[tb[it]][0] != None and C[tb[it]][0] != None):
            ts["Related_products,Color"] = []
            l1 = str(len(R[tb[it]]))
            l2 = str(len(C[tb[it]]))
            ts["Related_products,Color"].append("There are "+str(random.randint(int(l1)+1, int(
                l1)+4))+" related products to this but it comes in "+str(random.randint(int(l2)+1, int(l2)+4))+" colors")
            ts["Related_products,Color"].append("There are " + ("more" if int(
                l1) < int(l2) else "less")+" related products than color it comes in")
        # 2
        if (V[tb[it]][0] != None and M[tb[it]][0] != None):
            ...
            l1 = str(len(V[tb[it]]))
            l2 = str(len(M[tb[it]]))
            ts["Variants,Main_ingredients"].append("There are "+str(random.randint(int(l1)+1, int(
                l1)+4))+" variants of this but "+str(random.randint(int(l2)+1, int(l2)+3))+" necessary ingredients")
            ts["Variants,Main_ingredients"].append(
                "There are " + ("more" if int(l1) < int(l2) else "less")+" variants than necessary ingredients")
        if (V[tb[it]][0] != None and C[tb[it]][0] != None):
            ts["Variants,Color"] = []
            l1 = str(len(V[tb[it]]))
            l2 = str(len(C[tb[it]]))
            ts["Variants,Color"].append("There are "+str(random.randint(int(l1)+1, int(l1)+4)) +
                                        " variants of this but it comes in "+str(random.randint(int(l2)+1, int(l2)+4))+" colors")
            ts["Variants,Color"].append(
                "There are " + ("more" if int(l1) < int(l2) else "less")+" variants than color it comes in")
        # 3
        if (M[tb[it]][0] != None and C[tb[it]][0] != None):
            ...
            l1 = str(len(M[tb[it]]))
            l2 = str(len(C[tb[it]]))
            ts["Main_ingredients,Color"].append("There are "+str(random.randint(int(l1)+1, int(
                l1)+4))+" necessary ingredients to make this but it comes in "+str(random.randint(int(l2)+1, int(l2)+4))+" colors")
            ts["Main_ingredients,Color"].append("There are " + ("more" if int(
                l1) < int(l2) else "less")+" main ingredients than color it comes in")
...
def multi_row2(tb, dn, F, it, tval=True):
    ...
    if (tval):
        if (M[tb[it]][0] != None and V[tb[it]][0] != None):
            ...
            Al1 = ','.join(M[tb[it]])
            l1 = str(len(V[tb[it]]))
            ts["Manufacturer,Variants"].append(
                Al1+" manufactures "+l1+" variants of the product")
...
def multi_row3(tb, dn, F, it, tval=True):
    ...
    if (tval):
        ...
        if (T[tb[it]][0] != None and V[tb[it]][0] != None):
            ...
            Al1 = random.sample(T[tb[it]], 1)[0]
            NT = random.sample(
                list(set(Uv)-set(V[tb[it]])), random.randint(2, 4))
        ...
        if (T[tb[it]][0] != None and C[tb[it]][0] != None):
            ...
            Al1 = random.sample(T[tb[it]], 1)[0]
            NT = random.sample(
                list(set(Uc)-set(C[tb[it]])), random.randint(2, 4))
            Al2 = ",".join(random.sample(NT, random.randint(1, len(NT))))
            ts["Type,Color"].append(Al1+" comes in "+Al2+" colors")
