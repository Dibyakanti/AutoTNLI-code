...
table_index = np.array(
    category_map[category_map.category.isin(["Sports", "Sports Event"])].table_id)
...
def get_Venue(T, N, fake=False, sel=0):
                ...
                if (type(dictionary[k1]) == list):
                    ...
                    for i in range(length):
                        if (len(dictionary[k1].split(")")[i]) > 1):
                            u.add(dictionary[k1].split(")")[i].strip().strip(
                                ".").strip(",") + (")" if length > 1 else ""))
                            d[dictionary['Tablename']].append(dictionary[k1].split(")")[i].strip().strip(
                                ".").strip(",") + (")" if length > 1 else ""))
                ...
                if (type(dictionary[k2]) == list):
                    ...
                    for i in range(length):
                        if (len(dictionary[k2].split(")")[i]) > 1):
                            u.add(dictionary[k2].split(")")[i].strip().strip(
                                ".").strip(",") + (")" if length > 1 else ""))
                            d[dictionary['Tablename']].append(dictionary[k2].split(")")[i].strip().strip(
                                ".").strip(",") + (")" if length > 1 else ""))
    ...
    if (fake):
        for it in range(80):  # for getting all the fakes in one go
            sel = random.sample(
                FakeDICT_helper["SportsnEvents"]["Venue"], 1)[0]
...
def get_Competitors(T, N, fake=False, sel=0):
                ...
                if (type(dictionary[k]) == list):
                    ...
                    for i in range(len(dictionary[k].split(","))):
                        u.add(dictionary[k].split(",")[
                              i] .replace("\xa0", " "))
                        d[dictionary['Tablename']].append(
                            dictionary[k].split(",")[i] .replace("\xa0", " "))
    ...
    if (fake):
        for it in range(80):  # for getting all the fakes in one go
            sel = random.sample(
                FakeDICT_helper["SportsnEvents"][k.replace(" ", "_")], 1)[0]
            if (sel == 2 and len(d[T[it]]) < 2):
                sel = 1
            d = FakeDICT(T, N, u, d, it, sel)
    return list(u), d
...
def get_No_of_events(T, N, fake=False, sel=0):
    ...
    if (fake):
        for it in range(80):  # for getting all the fakes in one go
            sel = random.sample(
                FakeDICT_helper["SportsnEvents"]["No_of_events"], 1)[0]
            if (sel == 2 and len(d[T[it]]) < 2):
                sel = 1
            d = FakeDICT(T, N, u, d, it, sel)
    return list(u), d
...
def get_Established(T, N, fake=False, sel=0):
        ...
        if (int(table_index[n][1:]) <= 2800):
            ...
            if (k1 in dictionary.keys()):
                ...
                if (type(dictionary[k1]) == list):
                    ...
                    u.add(dictionary[k1].strip().strip(
                        ".") .replace("\xa0", " "))
                    d[dictionary['Tablename']].append(
                        dictionary[k1].strip().strip(".") .replace("\xa0", " "))
            ...
            if (k2 in dictionary.keys()):
                ...
                if (type(dictionary[k2]) == list):
                    ...
                    u.add(dictionary[k2].strip().strip(
                        ".") .replace("\xa0", " "))
                    d[dictionary['Tablename']].append(
                        dictionary[k2].strip().strip(".") .replace("\xa0", " "))
    ...
    if (fake):
        for it in range(80):  # for getting all the fakes in one go
            sel = random.sample(
                FakeDICT_helper["SportsnEvents"]["Established"], 1)[0]
            if (sel == 2 and len(d[T[it]]) < 2):
                sel = 1
            d = FakeDICT(T, N, u, d, it, sel)
...
def get_Data(fake=False):
    ...
    Extracted_data = {}
    Keys = ["Venue", "Date", "Competitors", "Teams",
            "No_of_events", "Established", "Official_site"]
    for k in Keys:
        Extracted_data[k] = []
...
def VenueSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                All = ','.join(di[tb[it]])
                ts.append("All the events of the sports " +
                          Nm+" are conducted in "+All)
                ts.append("Events of the sports happened in "+All)
                ts.append(random.sample(di[tb[it]], 1)[
                          0]+" was the official place where "+Nm+" happened")
                ts.append(Nm+" was held in "+random.sample(di[tb[it]], 1)[0])
                ts.append(Nm+" happened in " +
                          ("multiple" if length > 1 else "single")+" place")
...
def DateSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = ["It was held in "+All, All+" was when it was held"]
        else:
            ps1 = [None]
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                All = ','.join(di[tb[it]])
                ts.append("The sporting event "+Nm+" took place on " +
                          random.sample(di[tb[it]], 1)[0])
                ts.append(Nm+" was held over a few days")
                ts.append(Nm+" was a "+("single" if length <=
                          2 else "multiple")+" day event")

            else:
                NT = random.sample(
                    list(set(univ)-set(di[tb[it]])), random.randint(1, 2))
                All = ','.join(NT)
                ts.append("The sporting event "+Nm +
                          " took place on "+random.sample(NT, 1)[0])
                ts.append(Nm+" was held over a few days")
                ts.append(Nm+" was a "+("single" if length >
                          2 else "multiple")+" day event")
...
def CompetitorsSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = [All+" were the competitors in "+Nm, All+" competed in "+Nm]
        else:
            ps1 = [None]
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                All = ','.join(di[tb[it]])
                ts.append(
                    str(X)+random.sample([" people", " players"], 1)[0]+" competed in the "+Nm)
                ts.append("The sport "+Nm+" had " +
                          str(X)+random.sample(syn, 1)[0])
                ts.append("The sport "+Nm+" had more than " +
                          str(random.randint(5, X-4))+random.sample(syn, 1)[0])
                ts.append("The sport "+Nm+" had less than " +
                          str(random.randint(X+2, X+20))+random.sample(syn, 1)[0])
                if (length >= 2):
                    Y = re.findall("[0-9]+", ','.join(di[tb[it]]))[1]
                    Y = int(Y.strip())
                    ts.append(str(X)+random.sample(syn, 1)
                              [0]+"from "+str(Y)+" nations")
                    ts.append("In the sport "+Nm+" ," +
                              str(Y)+" nations participated")
                    ts.append("More than "+str(random.randint(0, Y-1)
                                               )+" nations participated in the event")
                    ts.append("Less than "+str(random.randint(Y+5, Y+10)
                                               )+" nations participated in the event")
                    ts.append(str(Y)+" nations took part in "+Nm)
                    ts.append("The sport "+Nm+" has "+str(X)+random.sample(syn, 1)
                              [0]+" from more than "+str(random.randint(0, Y-1))+" nations")
                    ts.append("The sport "+Nm+" has "+str(X)+random.sample(syn, 1)
                              [0]+" from less than "+str(random.randint(Y+2, Y+20))+" nations")
                    ts.append("The sport "+Nm+" has more than"+str(random.randint(5,
                              X-4))+random.sample(syn, 1)[0]+" from "+str(Y)+" nations")
                    ts.append("The sport "+Nm+" has less than"+str(random.randint(X+2,
                              X+20))+random.sample(syn, 1)[0]+" from "+str(Y)+" nations")
                    ts.append("The sport "+Nm+" has more than"+str(random.randint(5, X-4))+random.sample(
                        syn, 1)[0]+" from more than "+str(random.randint(0, Y-1))+" nations")
                    ts.append("The sport "+Nm+" has less than"+str(random.randint(X+4, X+20))+random.sample(
                        syn, 1)[0]+" from less than "+str(random.randint(Y+2, Y+20))+" nations")
            else:
                X = re.findall("[0-9]+", ','.join(di[tb[it]]))[0]
                X = int(X.strip())
                nX = random.randint(X+1, X+7)
                ts.append(
                    str(nX)+random.sample([" people", " players"], 1)[0]+" competed in the "+Nm)
                ts.append("The sport "+Nm+" had " +
                          str(nX)+random.sample(syn, 1)[0])
                ts.append("The sport "+Nm+" had less than " +
                          str(random.randint(5, X-4))+random.sample(syn, 1)[0])
                ts.append("The sport "+Nm+" had more than " +
                          str(random.randint(X+2, X+20))+random.sample(syn, 1)[0])
                if (length >= 2):
                    ...
                    ts.append(str(nX)+random.sample(syn, 1)
                              [0]+"from "+str(nY)+" nations")
                    ts.append("In the sport "+Nm+" ," +
                              str(nY)+" nations participated")
                    ts.append("Less than "+str(random.randint(0, Y-1)
                                               )+" nations participated in the event")
                    ts.append("More than "+str(random.randint(Y+5, Y+10)
                                               )+" nations participated in the event")
                    ts.append(str(Y)+" nations took part in "+Nm)
                    ts.append("The sport "+Nm+" has "+str(nX)+random.sample(syn, 1)
                              [0]+" from less than "+str(random.randint(0, Y-1))+" nations")
                    ts.append("The sport "+Nm+" has "+str(nX)+random.sample(syn, 1)
                              [0]+" from more than "+str(random.randint(Y+2, Y+20))+" nations")
                    ts.append("The sport "+Nm+" has less than"+str(random.randint(5,
                              X-4))+random.sample(syn, 1)[0]+" from "+str(Y)+" nations")
                    ts.append("The sport "+Nm+" has more than"+str(random.randint(X+2,
                              X+20))+random.sample(syn, 1)[0]+" from "+str(Y)+" nations")
                    ts.append("The sport "+Nm+" has less than"+str(random.randint(5, X-4))+random.sample(
                        syn, 1)[0]+" from less than "+str(random.randint(0, Y-1))+" nations")
                    ts.append("The sport "+Nm+" has more than"+str(random.randint(X+4, X+20))+random.sample(
                        syn, 1)[0]+" from more than "+str(random.randint(Y+2, Y+20))+" nations")
...
def TeamsSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            All = ','.join(di[tb[it]])
            ps1 = [All+" teams "+random.sample(["competed", "participated"], 1)[
                0]+" in "+Nm, All+" were the number of teams who competed in "+Nm]
        else:
            ps1 = [None]
        return ps1
    else:
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                ts.append("The number of"+random.sample(syn, 1)
                          [0]+"in the sport "+Nm+" is "+str(X))
                ts.append("There are "+str(X) +
                          random.sample(syn, 1)[0]+"in sport "+Nm)
                ts.append("There are more than "+str(random.randint(2,
                          X-4))+random.sample(syn, 1)[0]+"in sport "+Nm)
                ts.append("There are less than "+str(random.randint(X+4,
                          X+10))+random.sample(syn, 1)[0]+"in sport "+Nm)
                ts.append(str(X)+" teams competed against each other in "+Nm)
                ts.append(Nm+" saw a competition between " +
                          str(X)+random.sample(syn, 1)[0])

            else:
                nX = random.randint(X+1, X+7)
                ts.append("The number of"+random.sample(syn, 1)
                          [0]+"in the sport "+Nm+" is "+str(nX))
                ts.append("There are "+str(nX) +
                          random.sample(syn, 1)[0]+"in sport "+Nm)
                ts.append("There are less than "+str(random.randint(2,
                          X-4))+random.sample(syn, 1)[0]+"in sport "+Nm)
                ts.append("There are more than "+str(random.randint(X+4,
                          X+10))+random.sample(syn, 1)[0]+"in sport "+Nm)
...
def No_of_eventsSent(tb, dn, F, it, tval=True, prem=False):
    ...
    if (prem):
        if (di[tb[it]][0] != None):
            ...
            ps1 = ["The no. of events in sport was " +
                   str(X), str(X)+" events were there in "+Nm]
        else:
            ps1 = [None]
        return ps1
    else:
        ...
        if (di[tb[it]][0] != None):
            X = int(di[tb[it]][0].strip().strip("."))
            if (tval):
                ts.append("There were "+str(X) +
                          random.sample(syn, 1)[0]+"in "+Nm)
                ts.append("There were more than "+str(random.randint(1,
                          X-2))+random.sample(syn, 1)[0]+"in "+Nm)
                ts.append("There were less than "+str(random.randint(X +
                          2, X+8))+random.sample(syn, 1)[0]+"in "+Nm)
                ts.append(str(X)+random.sample(syn, 1)[0]+"were held in "+Nm)
                ts.append("More than "+str(random.randint(1, X-2)) +
                          random.sample(syn, 1)[0]+"were held in "+Nm)
                ts.append("Less than "+str(random.randint(X+2, X+8)) +
                          random.sample(syn, 1)[0]+"were held in "+Nm)

            else:
                nX = random.randint(X+1, X+7)
                ts.append("There were "+str(nX) +
                          random.sample(syn, 1)[0]+"in "+Nm)
                ts.append("There were less than "+str(random.randint(1,
                          X-2))+random.sample(syn, 1)[0]+"in "+Nm)
                ts.append("There were more than "+str(random.randint(X +
                          2, X+8))+random.sample(syn, 1)[0]+"in "+Nm)
                ts.append(str(nX)+random.sample(syn, 1)[0]+"were held in "+Nm)
                ts.append("Less than "+str(random.randint(1, X-2)) +
                          random.sample(syn, 1)[0]+"were held in "+Nm)
                ts.append("More than "+str(random.randint(X+2, X+8)) +
                          random.sample(syn, 1)[0]+"were held in "+Nm)
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                ...
                ts.append("In the year "+str(year) +
                          " the sport was played for the first time")
                ts.append("The sport event never happened before "+str(year))
                ts.append(Nm+" was"+random.sample(syn, 1)
                          [0]+str(2020-year)+" years ago")
                for i in jub.keys():
                    if (year+i <= 2020):
                        ts.append(Nm+" event has celebrated it's " +
                ...
                ts.append(Nm+" was started on "+All)
                ts.append("In the year "+str(nyear) +
                          " the sport was played for the first time")
                ts.append("The sport event never happened before "+str(nyear))
                ts.append(Nm+" was"+random.sample(syn, 1)
                          [0]+str(2020-year+random.randint(5, 40))+" years ago")
                for i in jub.keys():
                    if (nyear+i <= 2020):
                        ts.append(Nm+" event has celebrated it's " +
                                  jub[i]+"jubilee on "+str(nyear+i))
                    else:
                        ts.append(Nm+" will celebrate it's " +
                                  jub[i]+"jubilee on "+str(nyear+i))
    ...
    if (prem):
        ...
        if (di[tb[it]][0] != None):
            ...
            if (tval):
                ...
                All = ','.join(NT)
                ndom = random.sample(
                    list(set(udom)-set(dom)), random.randint(1, 2))
                ts.append("The website of the sport is "+All)
                ts.append("The website has domain name " +
                          random.sample(ndom, 1)[0])
                ts.append("Information about "+Nm+" could be found at "+All)
